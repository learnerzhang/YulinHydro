      ...
      <div v-if="searchResults" class="search-results">
        <div v-if="searchResults.length > 0">
          ...
          <div class="result-list">
            <div v-for="(news, index) in searchResults" :key="index" class="result-item">
              <a :href="news.url" target="_blank" class="result-title">{{ news.title }}</a>
              <div class="result-url">{{ formatUrl(news.url) }}</div>
              <div class="result-description">{{ news.description }}</div>
              <div class="result-meta">
                <span class="result-date">{{ news.date }}</span>
...
const handleSearch = async () => {
  ...
  try {
    const res = await apis.searchTechNews(
      searchQuery.value, 
      currentPage.value, 
      itemsPerPage.value,
    ...
    if (res.status === 200) {
      ...
      totalResults.value = res.data.count
      totalPages.value = Math.ceil(res.data.count / itemsPerPage.value)
    } else {
      searchResults.value = []
